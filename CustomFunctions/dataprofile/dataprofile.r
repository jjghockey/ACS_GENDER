...
dataprofile<-function(dta) {
		...
		#A. Summary Statistics
			#1. Identify Characters 
				fac<-as.data.frame.matrix(summary.default(dta))
		...
		#B. Structure 
			st1<-capture.output(str(dta, list.len = ncol(dta)))
			st1<-substr(st1,regexpr(":", st1),255)
			st1<-trim(tolower(gsub(":", "",st1)))
